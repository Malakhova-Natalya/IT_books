...
`Slightly Ragged/Variable Depth Hierarchies`. Такие иерархии не имеют фиксированного количества уровней, но дальность глубины небольшая. У географических иерархий часто уровень глубины колеблется от 3 до 6 уровней. Такие данные можно разместить в отдельных атрибутах измерений с максимальным количеством уровней, и заполнить их значениями исходя из бизнес-правил.

**Иерархические структуры с небольшой вариативностью глубины уровней и таблицы-мосты иерархий**

`Ragged/Variable Depth Hierarchies with Hierarchy Bridge Tables`. Иерархические структуры с вариативной глубиной сложны для моделирования и запросов в реляционных базах данных. Эти сложности можно обойти при помощи таблицы-моста. Такая таблица содержит строку для каждого возможного пути в иерархии и такими данными можно пользоваться при помощи стандартного SQL.

**Иерархические структуры с небольшой вариативностью глубины уровней и строковые атрибуты пути**

`Ragged/Variable Depth Hierarchies with Pathstring Attributes`. Вместо таблицы-моста можно использовать вставиваемый в измерение строковый атрибут пути. Для каждой строки измерения, строковый атрибут пути содержит специально закодированный текст, содержащий полное описание пути сверху вниз. Таким подходом тоже можно пользоваться при помощи стандартного SQL. Но такой подход становится уязвимым при появлении каких-либо изменений.

### Advanced Fact Table Techniques
***(Передовые методы составления таблиц фактов)*** стр 58

Техники в этом разделе описывают менее распространенные паттерны.

**Суррогатные ключи таблицы фактов**

`Fact Table Surrogate Keys`. Суррогатные ключи таблицы фактов, не ассоциированные ни с каким измерением, создаются последовательно в ETL-процессе и используются:
1) в качестве столбца с первичным ключом таблицы фактов
2) в качестве мгновенного идентификатора строки (без использования измерений)
3) чтобы позволить прерванному процессу загрузки откатиться или перезапуститься
4) позволить таблице фактов обновляться декомпозированно - с менее рискованными вставками и удалениями

**Таблицы фактов типа «сороконожка»**

`Centipede Fact Tables`. Некоторые дизайнеры создают отдельные нормализованные измерения для каждого уровня иерархий многие-к-одному, таким как измерения даты, месяца, квартала, года, и затем включают все эти внешние ключи в таблицу фактов. Таким образом получается таблица фактов типа "сороконожка" с десятками иерархически связанных измерений. Такого следует избегать. Необходимо окатить такие данные назад к их изначальной гранулярности - например, дате. 

**Числовые значения как атрибуты или факты**

`Numeric Values as Attributes or Facts`. Дизайнеры иногда сталкиваются с числовыми значениями, которые не очевидно куда подходят - то ли к таблице фактов, то ли к таблице измерений. Классический пример - стандартная цена продукта.

Если числовое значение используется в первую очередь в расчётах, скорее это принадлежит к фактам. Если постоянное числовое значение используется преимущественно для фильтрации и группировки, с ним следует обращаться как с атрибутом измерения. Иногда (в неокторых случаях) полезно моделировать числовое значение и как атрибут факта, и как атрибут измерения.

**Факты длительности/задержки**

`Lag/Duration Facts`. Аккумулирующие снимки данных таблицы фактов захватывают множество этапов (`milestones`), каждый из которых - с датой во внешнем ключе и возможно с отметкой даты/времени. Бизнес-пользователи часто хотят анализировать задержки или длительности между этими эапами. Вместо того чтобы нагружать пользовательские запросы расчётами, можно самим рассчитать все отличия между этапами.

**Таблица фактов заголовка/таблица фактов строки**

`Header/Line Fact Tables`. Операционные транзакционные системы часто состоят из строки-заголовка транзакции, каоторая ассоциирована с множеством транзационных строк. Со схемой `header/line` (также известной как `parent/child`), все внешние ключи известных измерений-заголовков и вырожденных измерений должны быть включены в таблицу фактов строки.

**Распределенные факты**

`Allocated Facts`. Довольно обыкновенно в транзакционных данных заголовка/строки встретить факты с различной гранулярностью. Вы должны стремиться разместить данные заголовка на уровень строки, чтобы этими фактами можно пользоваться любым способом сквозь призму измерений.

**Таблицы прибылей и убытков с использованием распределения средств**

`Profit and Loss Fact Tables Using Allocations`. Таблицы фактов, которые представляют полное уравнение прибыли - один из главных результатов DW/BI системы. Уравнение прибыли это `revenue - costs = profit`. В идеале таблицы фактов обеспечивают уравнение прибыли на уровне гранулярности транзакций дохода и содержат много компонентов затрат. Поскольку такие таблицы имеют самыую детальную грануляцию, из них можно получить множество сводных данных (включая прибыль с клиента, прибыль с продукта и тд). Такие таблицы сложно построить, поскольку компоненты должын ыть извдечены из источников и приведены к нужной гранулярности.

**Факты о нескольких валютах**

`Multiple Currency Facts`. Таблицы фактов, которые записывают финансовые транзакции в нескольких валютах, должны содержать пару столбцов для каждого финансового факта в строке. Один столбец содержит факт, выраженный в настоящей валюте транзакции, другой - содержит этот же факт, выраженный в единой стандартной валюте, использующейся по всей таблице. Такая таблица фактов также должна иметь измерение валют, чтобы идентифицировать настоящую (исходную) валюту транзакции.

**Факты с множественными единицами измерения**

`Multiple Units of Measure Facts`. Некоторые бизнес-процессы требуют, чтобы факты были заданы одновременно в нескольких единицах измерений.Для примера, в зависимости от нужд бизнес-пользователя, цепочка поставок может иметь необходимость предоставлять данные об одних и тех же фактах как о поддонах, транспортировочных ящиках, розничных ящиках и тд. Если таблица фактов содержит множество фактов, каждый из которых должен быть выражен во всех единицах измерения, удобная техника - записывать данные единожды в согласованном стандарте измерений, и одновременно хранить факторы конверсии в стандартную единицу измерений всех остальных.

**Факты с начала года**

`Year-to-Date Facts`. Бизнес-пользователи часто запрашивают факты с начала года в таблице фактов (также могут фигурировать фискальный период, фискальный период к дате). Такое лучше делать на стороне BI или OLAP кубов, но не в самой таблице фактов.

**Многопроходный SQL для избежания соединений таблиц фактов**

`Multipass SQL to Avoid Fact-to-Fact Table Joins`. BI-приложению не надо никогда выполнять SQL, соединяющий две таблицы фактов через внешние ключи таблиц фактов. Невозможно контролировать кардинальность ответа, могут быть возвращены неверные результаты. Например, если две таблицы фактов содержат покупки и возвраты, эти две таблицы фактов не должны напрямую соединяться через ключи покупателя и продукта. Вместо этого техника "бурления вдоль" (`drilling across`) двух таблиц фактов должна быть применена, где отдельно добывается результат от покупок и отдельно от возвратов, а затем эти результаты уже соединяются между собой для вывода корректного результата.

**Отслеживание временных промежутков в таблицах фактов**

`Timespan Tracking in Fact Tables`. Существует 3 базовых вида таблиц фактов:
1) транзакционные (`transaction`)
2) периодический снимок данных (`periodic snapshot`)
3) накопительный снимок данных (`accumulating snapshot`)

В отдельных кейсах полезно добавить `row effective date`, `row expiration date`, `current row indicator` к таблице фактов (как в SCD типа 2), чтобы поймать временной промежуток, когда значение в строке было валидным.

**Факты с задержкой поступления**

`Late Arriving Facts`. Строка фактов считается поступившей с опозданием, если текущий измерительный контекст для новых строк фактов не подходит к новой строке. Такое случается, когда строка фактов задерживается. В этом случае, должны быть найдены релевантные измерения и ключи измерений, которые были валидны на момент, когда случилось то пришедшее с задержкой событие.


### Advanced Dimension Techniques
***(Передовые методы измерения)*** стр 62

В этом разделе рассматриваются более продвинутые паттерны таблиц измерений.

**Соединения между таблицами измерений**

`Dimension-to-Dimension Table joins`. Измерения могут содержать ссылки на другие измерения. Хотя эти связи могут быть смоделированы с боковыми измерениями (`outrigger dimensions`), в некоторых случаях существование внешнего ключа к боковому измерению в базовом измерении может стать причиной взрывного роста (`expolsive growth`) базового измерения (поскольку изменения типа 2 в боковом измерении провоцируют соответствующие изменения типа 2 в базовом измерении). Этот взрывной рост часто можно избежать за счёт смещения корреляции между измерениями - размещая внешний ключ бокового измерения в таблицу фактов, а не в базовое измерение. Это значит взаимосвязь между измерениями может быть извлечена только через таблицу фактов.

**Многозначные измерения и мостовые таблицы**

`Multivalued Dimensions and Bridge Tables`. В классической схеме измерений, каждое измерение, прикрепленное к таблице фактов, имеет единственное значение, согласующееся с гранулярностью таблицы фактов. Но есть набор ситуаций, когда измерение легитимно имеет несколько значений. Например, пациент может иметь несколько диагнозов. В таких ситуациях мультизначное измерение должно быть прикреплено к таблице фактов через групповой ключ измерения к таблице-мосту с одной строкой для каждого диагноза в группе.

**Промежуточные таблицы с изменяющимися во времени многозначными связями**

`Time Varying Multivalued Bridge Tables`. Многозначная таблица-мост нуждается в том, чтобы быть основанной на 2-м типе SCD (добавление новой строки).

**Поведенческие теги временных рядов**

`Behavior Tag Time Series`. Почти весь текст в хранилище данных - описательный текст в таблицах измерений. Например, измерения поведения пользователей сквозь время становятся последоательностью тегов поведения.

**Группы изучения поведения**

`Behavior Study Groups`. Сложное поведение пользователей иногда может быть исследовано только при помощи длинных итераций исследования. Вместо того чтобы нагружать вычислениями BI, можно результаты исследования комплексного поведения записывать в простую таблицу, называемую `study group`, состоящую только из неизменных (`durable`) ключей пользователей.

**Агрегированные факты как атрибуты измерений**

`Aggregated Facts as Dimension Attributes`. Бизнес-пользователи часто заинтересованы в ограничении измерения пользователей, основанном на агрегированных метриках производительности, например, фильтрация всех пользователей, кто потратил больше определнной суммы в течение прошлого года. Агрегированные факты могут быть расположены в измерении как цели для ограничения и лейблы для создания отчётности. Такие расчёты становятся ношей для ETL, но облегчают ношу BI.

**Динамические диапазоны значений**

`Dynamic Value Bands`. Отчёт с динамическими диапазонами значений организован как серия отчётов с заголовками, которые определяют прогрессивную шкалу вариативных рангов целевой числовой метрики. Например, для банковских данных: "баланс от 0 до 10$", "баланс от 10.01 до 25$" и т.д. Этот вид отчёта - динамический, поскольку заголовки задаются в самом запросе.

**Измерение текстовых комментариев**

`Text Comments Dimension`. Чтобы не засорять таблицу фактов текстовыми метриками в виде свободных по форме комментариев, нужно вывести такие даннные отдельно в таблицу измерений.

**Множество часовых поясов**

`Multiple Time Zones`. Чтобы получить и универсальное, и локальное время, надо создавать двойной внешний ключ в связанные с такими данными таблицы фактов, которые в свою очередь связываются с таблицами измерений.

**Измерительные типовые измерения**

`Measure Type Dimensions`. Иногда, когда таблица фактов имеет длинный список фактов, редко заполненный какими-то индивидуальными строками, есть соблазн создать такое типовое измерение (`measure type dimension`), которое схлопывает строки фактов в единый сгенерированный факт, определяемый таким типовым измерением. Такой подход не рекомендуется.

**Шаговые измерения**

`Step Dimensions`. Последовательные события, такие как события на веб-странице, обычно имеют отдельную строку в транзакционной таблице фактов для каждого шага в процессе. Чтобы рассказать, где индивидуальный шаг сходится со всей сессией, применяется шаговое измерение - оно показывает, какой номер шага представляет собой выбранный шаг и как много шагов ещё входит во всю сессию.

**Горячезаменяемые измерения**

`Hot Swappable Dimensions`. Такие измерения применяются, когда одна и та же таблица фактов альтернативно соединяется с разными копиями измерения. Например, таблица фактов, содержащая котировки акций может использоваться различными инвесторами.

**Абстрактные универсальные измерения**

`Abstract Generic Dimensions`. Иногда вместо отдельных измерений (например, географических атрибутов магазина, покупателя) создают одно. Скажем, измерение пользователя: вместо измерений работников, продавцов, покупателей создаётся одно измерение про всех людей, несмотря на разность их ролей и различие в сути их атрибутов. Такого подхода следует избегать.

**Измерение аудита**

`Audit Dimensions`. Когда в ETL создаётся строка таблицы фактов, полезно создать измерение аудита, содержащее метадату ETL процесса.

**Отстающие измерения**

`Late Arriving Dimensions`. Иногда факты из операционного бизнес-процесса прибывают раньше, чем связанный измерительный контекст. И такие факты уже показываются в BI. В таких случаях следует использовать специальные строки измерений, а далее уже заполнять данные прибывшими настоящими измерениями (тип 2 SCD - перезапись строк).

### Special Purpose Schemas
***(Схемы специального назначения)*** стр 67

Этот раздел - для специфических случаев.

**Схемы супертипа и подтипа для разнородных продуктов**

`Supertype ans Subtype Schemas for Heterogeneous Products`. Бизнесы часто предлагают широкое разнообразие продуктов в разных направлениях. Например, банк может предлагать десятки различных видов счетов, но всё это будут счета. Попытки создать единую таблицу фактов для всех возможных видов фактов, связанную с таблицей измерений, имеющую все возможные виды атрибутов - такая попытка приведёт к краху. Решение - создать единую `supertype fact table`, и затем построить отдельные таблицы фактов для каждого подтипа. Супертиповые и подтиповые таблицы фактов также называют `core` и `custom` таблицами фактов.

**Таблицы фактов в реальном времени**

`Real-Time Fact Tables`. Такие таблицы требуют обновления более частого, чем традиционное ночное обновление. Есть техники, позволяющие выполнить это требование, и они зависят от особенностей DBMS или OLAP кубов. Например, можно выделять "горячую" партицию.

**Схема событий ошибок**

`Error Event Schemas`. Для отслеживания качества данных может быть создана отдельная схема внутри ETL-системы. Когда проверка качества данных находит ошибку, она записывает строку в специальной схеме в таблице фактов ошибок. В этой строке все детали по ошибке.
